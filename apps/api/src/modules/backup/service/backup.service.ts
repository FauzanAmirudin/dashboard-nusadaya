...
export class BackupService {
	 ...
	 */
	async createBackupJob(params: CreateBackupParams): Promise<{ jobId: string; status: string; message?: string }> {
	...
	async getBackupStatus(jobId: string): Promise<object | null> {
		...
		return {
			...
			percentage: progress?.percentage ?? (record.status === "completed" ? 100 : 0),
	...
	async executeBackup(
		...
		if (!locked) {
			await backupRepository.markFailed(jobId, "Backup lain dengan filter sama sedang berjalan");
			return;
		}
		...
		try {
			...
			await setJobProgress("backup", jobId, { status: "processing", startedAt: new Date().toISOString() });

			// 1. Tentukan student IDs berdasarkan filter (serta peta id->nim untuk penamaan folder)
			const { studentIds, studentNimMap } = await this.resolveStudentIds(filters);
			...
			const nim = filters.nim ?? (filters.studentId ? (studentNimMap.get(filters.studentId) ?? String(filters.studentId)) : undefined);
			...
			for (const fileRecord of fileRecords) {
				...
				try {
					await fileService.copyFileToBackup(fileRecord.id, outputDir, relDestPath);
					...
					console.warn(`[Backup] Skip file ${fileRecord.id}: file tidak ditemukan di storage`);
				}
			...
			await backupRepository.markCompleted(jobId, outputDir, totalFiles, totalSize, processed);
	 ...
	 */
	private async resolveStudentIds(filters: BackupFilter): Promise<{ studentIds: number[]; studentNimMap: Map<number, string> }> {
		...
		if (filters.nim) {
			...
			if (!student) throw new Error(`Mahasiswa dengan NIM ${filters.nim} tidak ditemukan`);
			nimMap.set(student.id, student.nim!);
			return { studentIds: [student.id], studentNimMap: nimMap };
		}

		const allStudents = await db.query.students.findMany({
			columns: { id: true, nim: true, cohort: true, program: true, subProgram: true },
		});
		...
		const filtered = allStudents.filter((s) => {
			...
			if (filters.specializationId && s.subProgram !== filters.specializationId) return false;
			return true;
		});
