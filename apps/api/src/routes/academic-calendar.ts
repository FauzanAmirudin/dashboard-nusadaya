...
export const academicCalendarRoutes = new Elysia({ prefix: "/academic-calendars" })
	...
	.get("/", async ({ query }) => {
		...
		if (academicYear) conditions.push(eq(academicCalendars.academicYear, academicYear as string));
		if (cohort) conditions.push(eq(academicCalendars.cohort, parseInt(cohort as string, 10)));
		...
		const allData = await db.query.academicCalendars.findMany({ where: whereClause });
	...
	.get("/:id", async ({ params }) => {
		...
		const data = await db.query.academicCalendars.findFirst({
			...
			with: {
				periods: {
					orderBy: [asc(academicPeriods.orderIndex), asc(academicPeriods.startDate)],
				},
				events: {
	...
	// --- PERIODS ---
	.post("/:id/periods", async ({ params, body }) => {
		const calendarId = parseInt(params.id, 10);
		const payload = body as any;
	...
	.patch("/:id/periods/:periodId", async ({ params, body }) => {
		...
		if (payload.periodType !== undefined) updateData.periodType = payload.periodType;
		if (payload.startDate !== undefined) updateData.startDate = payload.startDate;
		if (payload.endDate !== undefined) updateData.endDate = payload.endDate;
	...
	// --- EVENTS ---
	.post("/:id/events", async ({ params, body }) => {
		const calendarId = parseInt(params.id, 10);
		const payload = body as any;
	...
	.patch("/:id/events/:eventId", async ({ params, body }) => {
