...
export const studentsRouter = new Elysia({ prefix: "/students" })
	...
	.get("/:id/status", async ({ params, set }) => {
		...
		const [pmb, crm, finance, academic, pa, internship, courses, student] =
			await Promise.all([
		...
		const finChecks = [
			finance?.registrationPaid,
			finance?.semesterPaid,
			finance?.installmentCleared,
			finance?.arrearsCleared,
		];
		...
		const panels = [
			{
				...
				total: pmbTotal,
				status: getStatus(pmbCompleted, pmbTotal),
			},
			{
				id: "crm",
				name: "CRM",
				...
				total: crmTotal,
				status: getStatus(crmCompleted, crmTotal),
			},
			{
				id: "finance",
				name: "Finance",
				completed: finCompleted,
				total: finTotal,
				status: getStatus(finCompleted, finTotal),
			},
			{
				id: "akademik",
				name: "Akademik",
				completed: acdCompleted,
				total: acdTotal,
				status: getStatus(acdCompleted, acdTotal),
			},
			{
				id: "dosen",
				name: "Dosen per MK",
				...
				total: courseTotal,
				status: getStatus(courseCompleted, courseTotal),
			},
			{
				id: "pa",
				name: "PA",
				...
				total: paTotal,
				status: getStatus(paCompleted, paTotal),
			},
			{
				id: "magang",
				name: "Tim Magang",
				...
				total: intTotal,
				status: getStatus(intCompleted, intTotal),
			},
		];
		...
		if (!crm?.practiceAttendance)
			incompleteIndicators.push({
				panel: "CRM",
				name: "Kehadiran Praktek",
				status: "PERLU_PERHATIAN",
				link: "crm",
			});
		if (!crm?.odsDocumentation)
			incompleteIndicators.push({
				panel: "CRM",
				name: "Dokumentasi ODS",
				status: "PERLU_PERHATIAN",
				link: "crm",
			});

		if (!finance?.registrationPaid)
			incompleteIndicators.push({
				panel: "Finance",
				name: "Daftar Ulang Belum Lunas",
				status: "TIDAK_AMAN",
				link: "finance",
			});
		if (!finance?.semesterPaid)
			incompleteIndicators.push({
				panel: "Finance",
				name: "Semester Belum Lunas",
				status: "TIDAK_AMAN",
				link: "finance",
			});
		if (!finance?.installmentCleared)
			incompleteIndicators.push({
				panel: "Finance",
				name: "Cicilan Belum Lunas",
				status: "TIDAK_AMAN",
				link: "finance",
			});
		if (!finance?.arrearsCleared)
			incompleteIndicators.push({
				panel: "Finance",
		...
		if (!academic?.attitudeIndicator)
			incompleteIndicators.push({
				panel: "Akademik",
				name: "Indikator Sikap",
				status: "PERLU_PERHATIAN",
				link: "akademik",
			});
		if (!academic?.assignmentsCompleted)
			incompleteIndicators.push({
				panel: "Akademik",
				name: "Tugas Selesai",
				status: "PERLU_PERHATIAN",
		...
		if (!academic?.academicCommunication)
			incompleteIndicators.push({
				panel: "Akademik",
		...
		if (unaccCourses.length > 0) {
			incompleteIndicators.push({
				panel: "Dosen",
				name: `Nilai ${unaccCourses.length} MK belum di-ACC`,
				status: "PERLU_PERHATIAN",
				link: "dosen",
			});
		}

		if (!pa?.counselingDone)
			incompleteIndicators.push({
				panel: "PA",
				name: "Sesi Konseling",
				status: "PERLU_PERHATIAN",
		...
		if (!pa?.mentalStable)
			incompleteIndicators.push({
				panel: "PA",
				name: "Kestabilan Mental",
				status: "PERLU_PERHATIAN",
				link: "pa",
			});
		if (!pa?.disciplineGood)
			incompleteIndicators.push({
				panel: "PA",
				name: "Kedisiplinan",
				status: "PERLU_PERHATIAN",
		...
		const hasTidakAman = incompleteIndicators.some(
			(i) => i.status === "TIDAK_AMAN",
		);
		if (hasTidakAman) {
		...
		if (student.overallStatus !== overallStatus) {
			await db
				.update(students)
				.set({ overallStatus: overallStatus as any })
				.where(eq(students.id, id));
		}
	...
	.patch(
		...
		async (context) => {
			...
			if (!current) {
				...
				if (cleanUpdates.registrationDate)
					cleanUpdates.registrationDate = new Date(
						cleanUpdates.registrationDate,
					);
				if (cleanUpdates.semesterDate)
	...
	.patch(
		...
		async (context) => {
			...
			// Recalculate status
			const current = await db.query.paData.findFirst({
				where: eq(paData.studentId, id),
			});
			if (current) {
				const isAman =
					current.counselingDone &&
					current.mentalStable &&
					current.disciplineGood;
				const isKritis = !current.mentalStable && !current.disciplineGood;

				const newStatus = isAman
					? "AMAN"
					: isKritis
						? "TIDAK_AMAN"
						: "PERLU_PERHATIAN";
	...
	.post(
		...
		async (context) => {
			...
			const logs = await db.query.counselingLogs.findMany({
				where: eq(counselingLogs.studentId, id),
			});
			if (logs.length >= 3) {
				await db
