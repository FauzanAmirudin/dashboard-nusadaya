...
export const attendanceRoutes = new Elysia({ prefix: "/attendance" })
	...
	.get("/sessions", async ({ query }) => {
		...
		const conditions = [];
		if (cohort) conditions.push(eq(attendanceSessions.cohort, parseInt(cohort as string, 10)));
		if (sessionType && sessionType !== "all") conditions.push(eq(attendanceSessions.sessionType, sessionType as string));
		if (dateFrom) conditions.push(sql`${attendanceSessions.sessionDate} >= ${dateFrom}`);
		if (dateTo) conditions.push(sql`${attendanceSessions.sessionDate} <= ${dateTo}`);
		
		if (scheduleId) {
			const idNum = parseInt(scheduleId as string, 10);
			if (sessionType === "kelas") conditions.push(eq(attendanceSessions.classScheduleId, idNum));
			if (sessionType === "praktikum") conditions.push(eq(attendanceSessions.practicumScheduleId, idNum));
			if (sessionType === "piket") conditions.push(eq(attendanceSessions.dutyScheduleId, idNum));
		}
		...
		const enrichedData = data.map(session => {
			const totalHadir = session.records.filter(r => r.status === "hadir").length;
			return {
				...session,
	...
	.post("/sessions", async ({ body, user }) => {
		...
		const result = await db.insert(attendanceSessions).values({
			...
			room: payload.room,
			notes: payload.notes,
			classScheduleId: payload.classScheduleId ? parseInt(payload.classScheduleId, 10) : null,
			practicumScheduleId: payload.practicumScheduleId ? parseInt(payload.practicumScheduleId, 10) : null,
			dutyScheduleId: payload.dutyScheduleId ? parseInt(payload.dutyScheduleId, 10) : null,
			createdBy: user.id,
		}).returning();
	...
	.patch("/sessions/:id", async ({ params, body }) => {
		...
		const result = await db.update(attendanceSessions).set(updateData).where(eq(attendanceSessions.id, id)).returning();
		return { success: true, data: result[0] };
	})
	.delete("/sessions/:id", async ({ params }) => {
		await db.delete(attendanceSessions).where(eq(attendanceSessions.id, parseInt(params.id, 10)));
		return { success: true };
	})
	...
	.post("/sessions/:id/records/bulk", async ({ params, body, user }) => {
		...
		const { records } = body as { records: Array<{ studentId: number; status: string; notes?: string }> };
	...
	.patch("/academic-data/:studentId", async ({ params, body }) => {
		...
		if (payload.attendanceTotal !== undefined) updateData.attendanceTotal = payload.attendanceTotal;
		if (payload.attendancePresent !== undefined) updateData.attendancePresent = payload.attendancePresent;
		if (payload.attendancePiketTotal !== undefined) updateData.attendancePiketTotal = payload.attendancePiketTotal;
		if (payload.attendancePiketPresent !== undefined) updateData.attendancePiketPresent = payload.attendancePiketPresent;
		if (payload.attendanceOdsTotal !== undefined) updateData.attendanceOdsTotal = payload.attendanceOdsTotal;
		if (payload.attendanceOdsPresent !== undefined) updateData.attendanceOdsPresent = payload.attendanceOdsPresent;
		if (payload.attendancePramagangTotal !== undefined) updateData.attendancePramagangTotal = payload.attendancePramagangTotal;
		if (payload.attendancePramagangPresent !== undefined) updateData.attendancePramagangPresent = payload.attendancePramagangPresent;
		...
		if (existingData) {
			const result = await db.update(academicData).set(updateData).where(eq(academicData.id, existingData.id)).returning();
			return { success: true, data: result[0] };
		} else {
			const result = await db.insert(academicData).values({
				studentId,
				...updateData
			}).returning();
			return { success: true, data: result[0] };
	...
	.get("/mahasiswa/:studentId/mata-kuliah", async ({ params }) => {
		...
		const mkRecords = records.filter(r => r.session.sessionType === "kelas" || r.session.sessionType === "praktikum");

		return { success: true, data: { grades, records: mkRecords } };
	})
	.patch("/mahasiswa/:studentId/mata-kuliah/:courseGradeId", async ({ params, body }) => {
		const courseGradeId = parseInt(params.courseGradeId, 10);
		const payload = body as any;
		const updateData: any = { updatedAt: new Date() };
		if (payload.attendancePresent !== undefined) updateData.attendancePresent = payload.attendancePresent;
		if (payload.totalMeetings !== undefined) updateData.totalMeetings = payload.totalMeetings;
		// Auto calculate rate
		if (payload.attendancePresent !== undefined && payload.totalMeetings !== undefined) {
			const rate = payload.totalMeetings > 0 ? Math.round((payload.attendancePresent / payload.totalMeetings) * 100) : 0;
			updateData.attendanceRate = rate;
		}
		const result = await db.update(courseGrades).set(updateData).where(eq(courseGrades.id, courseGradeId)).returning();
		return { success: true, data: result[0] };
	})
	.get("/mahasiswa/:studentId/piket", async ({ params }) => {
		...
		const piketRecords = records.filter(r => r.session.sessionType === "piket");
		...
		return { success: true, data: { records: piketRecords, schedules: studentSchedules } };
	})
	.post("/mahasiswa/:studentId/piket", async ({ params, body, user }) => {
		try {
			...
			if (payload.scheduleId) {
				sessionConditions.push(eq(attendanceSessions.dutyScheduleId, parseInt(payload.scheduleId, 10)));
			}
			...
			if (!session) {
				...
				const result = await db.insert(attendanceSessions).values({
					...
					subject: scheduleDetails ? `Piket ${scheduleDetails.groupName}` : "Piket Kebersihan",
	...
	.patch("/mahasiswa/:studentId/piket/:recordId", async ({ params, body }) => {
		...
		const studentId = parseInt(params.studentId, 10);
		const payload = body as any;
		await db.update(attendanceRecords).set({ status: payload.status, notes: payload.notes }).where(eq(attendanceRecords.id, recordId));
		await syncAttendanceAggregate(studentId);
		return { success: true };
	})
	// ODS Routes
	...
	.post("/mahasiswa/:studentId/ods", async ({ params, body, user }) => {
		const studentId = parseInt(params.studentId, 10);
		const payload = body as any;
		const result = await db.insert(odsAttendanceRecords).values({
			studentId,
			date: payload.date,
			status: payload.status,
			notes: payload.notes,
	...
	.patch("/mahasiswa/:studentId/ods/:recordId", async ({ params, body }) => {
		...
		const studentId = parseInt(params.studentId, 10);
		const payload = body as any;
		const result = await db.update(odsAttendanceRecords).set({
			date: payload.date,
			status: payload.status,
			notes: payload.notes
		}).where(eq(odsAttendanceRecords.id, recordId)).returning();
		await syncAttendanceAggregate(studentId);
		return { success: true, data: result[0] };
	})
	// Pramagang Routes
	...
	.post("/mahasiswa/:studentId/pramagang", async ({ params, body, user }) => {
		const studentId = parseInt(params.studentId, 10);
		const payload = body as any;
		const result = await db.insert(pramagangAttendanceRecords).values({
			studentId,
			date: payload.date,
			status: payload.status,
			notes: payload.notes,
	...
	.patch("/mahasiswa/:studentId/pramagang/:recordId", async ({ params, body }) => {
		const recordId = parseInt(params.recordId, 10);
		const studentId = parseInt(params.studentId, 10);
		const payload = body as any;
		const result = await db.update(pramagangAttendanceRecords).set({
			date: payload.date,
			status: payload.status,
			notes: payload.notes
		}).where(eq(pramagangAttendanceRecords.id, recordId)).returning();
		await syncAttendanceAggregate(studentId);
		return { success: true, data: result[0] };
	});
...
async function syncAttendanceAggregate(studentId: number) {
	...
	for (const rec of allRecords) {
		...
		if (type === "kelas" || type === "praktikum") {
			kelasPraktikumTotal++;
			if (rec.status === "hadir" || rec.status === "izin") kelasPraktikumHadir++;
		} else if (type === "piket") {
			piketTotal++;
	...
	let odsTotal = odsRecords.length;
	let odsHadir = odsRecords.filter(r => r.status === "hadir" || r.status === "izin").length;

	// Query Pramagang
	const pramagangRecords = await db.query.pramagangAttendanceRecords.findMany({
		where: eq(pramagangAttendanceRecords.studentId, studentId),
	});
	let pramagangTotal = pramagangRecords.length;
	let pramagangHadir = pramagangRecords.filter(r => r.status === "hadir" || r.status === "izin").length;
