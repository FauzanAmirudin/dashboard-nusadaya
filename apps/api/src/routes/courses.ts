...
export const coursesRoutes = new Elysia({ prefix: "/courses" })
	...
	.get("/", async ({ query, user, set }) => {
		...
		if (cohort) conditions.push(eq(courses.cohort, parseInt(cohort as string, 10)));
	...
	.delete("/:id", async ({ params, user, set }) => {
		...
		if (meetings.length > 0) {
			...
			await db.delete(courseMeetingActivities).where(inArray(courseMeetingActivities.meetingId, meetingIds));
			await db.delete(courseMeetingAttendances).where(inArray(courseMeetingAttendances.meetingId, meetingIds));
			await db.delete(courseMeetings).where(eq(courseMeetings.courseId, id));
		}

		await db.delete(courses).where(eq(courses.id, id));

		return { success: true, message: "Mata kuliah beserta jadwal mengajar berhasil dihapus" };
	})
	...
	.get("/:id/meetings", async ({ params, user, set }) => {
		...
		if (!course) { set.status = 404; return { success: false, message: "Not found" }; }
		if (user.role === "dosen" && course.dosenId !== user.id) { set.status = 403; return { success: false, message: "Forbidden" }; }
	...
	.patch("/:id/meetings/:meetingId", async ({ params, body, user, set }) => {
		...
		if (!meeting) { set.status = 404; return { success: false, message: "Not found" }; }
		if (user.role === "dosen" && meeting.course.dosenId !== user.id) { set.status = 403; return { success: false, message: "Forbidden" }; }

		const [updated] = await db.update(courseMeetings)
			.set({
	...
	// 3. ACTIVITIES CRUD
	// ==========================================
	.post("/:id/meetings/:meetingId/activities", async ({ params, body, user, set }) => {
		const meetingId = parseInt(params.meetingId, 10);
		const input = body as any;

		const [newActivity] = await db.insert(courseMeetingActivities).values({
			meetingId: meetingId,
		...
		body: t.Object({
			activityType: t.Enum({ teori: "teori", tugas: "tugas", praktik: "praktik", ujian: "ujian" }),
	...
	.patch("/:id/meetings/:meetingId/activities/:activityId", async ({ params, body, user, set }) => {
		const activityId = parseInt(params.activityId, 10);
		const input = body as any;

		const [updated] = await db.update(courseMeetingActivities)
			.set({
				activityType: input.activityType,
				score: input.score !== undefined ? input.score : undefined,
				notes: input.notes !== undefined ? input.notes : undefined,
				documentUrl: input.documentUrl !== undefined ? input.documentUrl : undefined,
	...
	.delete("/:id/meetings/:meetingId/activities/:activityId", async ({ params }) => {
		const activityId = parseInt(params.activityId, 10);
		await db.delete(courseMeetingActivities).where(eq(courseMeetingActivities.id, activityId));
	...
	// ==========================================
	.post("/:id/meetings/:meetingId/attendances", async ({ params, body, user, set }) => {
		const meetingId = parseInt(params.meetingId, 10);
		const input = body as any; // { attendances: [{ studentId: 1, status: "hadir", notes: "" }] }

		// Delete existing
		await db.delete(courseMeetingAttendances).where(eq(courseMeetingAttendances.meetingId, meetingId));
