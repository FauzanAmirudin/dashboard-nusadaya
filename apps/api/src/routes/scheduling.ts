...
export const schedulingRoutes = new Elysia({ prefix: "/scheduling" })
	...
	.get("/class-schedules", async ({ query }) => {
		...
		if (cohort)
			conditions.push(
				eq(classSchedules.cohort, parseInt(cohort as string, 10)),
			);
		if (dayOfWeek)
			conditions.push(eq(classSchedules.dayOfWeek, dayOfWeek as string));
		if (calendarId)
			conditions.push(
				eq(classSchedules.calendarId, parseInt(calendarId as string, 10)),
			);
		...
		async ({ body, set }) => {
			...
			const allRoomSchedules = [
				...
				(s) =>
					...
					(!payload.sessionDate ||
						!s.sessionDate ||
						payload.sessionDate === s.sessionDate),
			);
			...
			for (const s of allRoomSchedules) {
				if (
					...
					return {
						success: false,
						message: `Konflik Ruangan: Ruang ${payload.room} sudah terpakai pada rentang waktu ini.`,
					};
				}
			}
			...
			if (payload.dosenId) {
				const allDosenSchedules = [
					...existingClass,
					...existingPracticum,
				].filter(
					(s) =>
						s.dosenId === payload.dosenId &&
						(!payload.sessionDate ||
							!s.sessionDate ||
							payload.sessionDate === s.sessionDate),
				);
				...
				for (const s of allDosenSchedules) {
					if (
						...
						return {
							success: false,
							message: `Konflik Dosen: Dosen sudah memiliki jadwal mengajar pada rentang waktu ini.`,
						};
					}
				}
			}
			...
			const result = await db
				...
				.values({
					...
					calendarId: payload.calendarId
						? parseInt(payload.calendarId, 10)
						: null,
	...
	.patch("/class-schedules/:id", async ({ params, body }) => {
		...
		if (payload.calendarId !== undefined)
			updateData.calendarId = payload.calendarId
				? parseInt(payload.calendarId, 10)
				: null;
	...
	.delete("/class-schedules/:id", async ({ params }) => {
		await db
			...
			.where(eq(classSchedules.id, parseInt(params.id, 10)));
		return { success: true };
	})
	...
	.get("/practicum-schedules", async ({ query }) => {
		...
		if (cohort)
			conditions.push(
				eq(practicumSchedules.cohort, parseInt(cohort as string, 10)),
			);
		if (dayOfWeek)
			conditions.push(eq(practicumSchedules.dayOfWeek, dayOfWeek as string));
		if (calendarId)
			conditions.push(
				eq(practicumSchedules.calendarId, parseInt(calendarId as string, 10)),
			);
		...
		const data = await db.query.practicumSchedules.findMany({
			...
			orderBy: [
				asc(practicumSchedules.dayOfWeek),
				asc(practicumSchedules.startTime),
			],
		});
	...
	.patch("/practicum-schedules/:id", async ({ params, body }) => {
		...
		const result = await db
			.update(practicumSchedules)
			.set(updateData)
			.where(eq(practicumSchedules.id, id))
			.returning();
		return { success: true, data: result[0] };
	})
	.delete("/practicum-schedules/:id", async ({ params }) => {
		await db
			.delete(practicumSchedules)
			.where(eq(practicumSchedules.id, parseInt(params.id, 10)));
		return { success: true };
	})
	...
	.get("/duty-schedules", async ({ query }) => {
		...
		const conditions = [];
		if (cohort)
			conditions.push(eq(dutySchedules.cohort, parseInt(cohort as string, 10)));
		if (dayOfWeek)
			conditions.push(eq(dutySchedules.dayOfWeek, dayOfWeek as string));
	...
	.patch("/duty-schedules/:id", async ({ params, body }) => {
		...
		const result = await db
			...
			.set(updateData)
			.where(eq(dutySchedules.id, id))
			.returning();
		return { success: true, data: result[0] };
	})
	.delete("/duty-schedules/:id", async ({ params }) => {
		await db
			.delete(dutySchedules)
			.where(eq(dutySchedules.id, parseInt(params.id, 10)));
		return { success: true };
	})
		...
		async ({ body, user }) => {
			...
			const result = await db
				...
				.values({
					...
					targetCohort: payload.targetCohort
						? parseInt(payload.targetCohort, 10)
						: null,
	...
	.patch("/announcements/:id", async ({ params, body }) => {
		...
		if (payload.targetCohort !== undefined)
			updateData.targetCohort = payload.targetCohort
				? parseInt(payload.targetCohort, 10)
				: null;
	...
	.delete("/announcements/:id", async ({ params }) => {
		await db
			...
			.where(eq(announcements.id, parseInt(params.id, 10)));
		return { success: true };
	});
