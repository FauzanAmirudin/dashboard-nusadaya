...
type MeetingData = {
	...
	activities: any[];
	attendances: { studentId: number; status: string; notes: string | null; student: { name: string; nim: string } }[];
};
...
export default function MataKuliahDetailPage() {
	...
	const [meetingForm, setMeetingForm] = useState({ label: "", date: "", desc: "" });

	// Inline editing state for Activities
	const [editingActivity, setEditingActivity] = useState<{ meetingId: number, type: string, actId?: number } | null>(null);
	...
	// Attendance State
	const [attendanceData, setAttendanceData] = useState<Record<number, Record<number, { status: string, notes: string }>>>({});
	...
	const fetchData = async () => {
		...
		try {
			...
			// Get meetings
			const { data: mData, error: mErr } = await api.courses[courseId].meetings.get();
			if (!mErr && mData?.success) {
				const meets = mData.data as any[];
				setMeetings(meets);

				// Prepare attendance state
				const attState: Record<number, Record<number, { status: string, notes: string }>> = {};
				meets.forEach(m => {
					attState[m.id] = {};
					m.attendances.forEach((a: any) => {
						attState[m.id][a.studentId] = { status: a.status, notes: a.notes || "" };
					});
				});
	...
	const saveMeetingInfo = async (meetingId: number) => {
		try {
			const { error } = await api.courses[courseId].meetings[meetingId.toString()].patch({
	...
	// Save Activity
	const saveActivity = async (meetingId: number, actType: string, actId?: number) => {
		try {
			...
			if (actId) {
				const { error } = await api.courses[courseId].meetings[meetingId.toString()].activities[actId.toString()].patch(payload);
				if (error) throw error;
			} else {
				const { error } = await api.courses[courseId].meetings[meetingId.toString()].activities.post(payload);
				if (error) throw error;
			}
	...
	const saveAttendances = async (meetingId: number) => {
		...
		try {
			...
			const { error } = await api.courses[courseId].meetings[meetingId.toString()].attendances.post({
				attendances: payload
			});
			if (error) throw error;
	...
	const handleExportDetail = () => {
		...
		exportToCSV(exportData, `Rekap_${course.code}_${course.name.replace(/\s+/g, '_')}`);
	};
	...
	if (!course) return <div className="p-6 text-center text-red-500">Mata kuliah tidak ditemukan</div>;
		...
		<div className="p-6 max-w-7xl mx-auto space-y-6">
				...
				<div className="flex items-center gap-3">
					...
					<div>
						<div className="flex items-center gap-2">
							...
							<Badge variant="outline" className="bg-slate-50">{course.code}</Badge>
						</div>
						<p className="text-slate-500">Dosen Pengampu: {course.dosen.fullName}</p>
					</div>
				</div>
			...
			<Card>
				<CardHeader className="pb-3 border-b">
					<CardTitle className="text-xl">Jadwal Mengajar & Jurnal Perkuliahan</CardTitle>
					<CardDescription>Pilih pertemuan untuk mengisi presensi, materi, dan nilai</CardDescription>
				</CardHeader>
				<CardContent className="p-4">
					<Accordion type="single" collapsible className="w-full space-y-2">
						{meetings.map((meet) => {
							const isEditingInfo = editingMeeting === meet.id;
							const isUtsUas = meet.meetingType === "uts" || meet.meetingType === "uas";

							return (
								<AccordionItem value={`meet-${meet.id}`} key={meet.id} className="border rounded-lg px-4 py-2 bg-white data-[state=open]:border-blue-200 data-[state=open]:shadow-sm transition-all">
									<AccordionTrigger className="hover:no-underline py-2">
										<div className="flex items-center gap-3 w-full justify-between pr-4">
											<div className="flex items-center gap-3 text-left">
												<div className={`p-2 rounded-full ${isUtsUas ? "bg-amber-100 text-amber-600" : "bg-blue-100 text-blue-600"}`}>
													{isUtsUas ? <FileText className="h-4 w-4" /> : <BookOpen className="h-4 w-4" />}
												</div>
												<div>
													<p className="font-semibold">{meet.meetingLabel}</p>
													<p className="text-sm text-slate-500 font-normal">
														{meet.meetingDate ? new Date(meet.meetingDate).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) : "Tanggal belum diatur"}
													</p>
												</div>
											</div>
											<Badge variant="secondary" className="font-normal shrink-0">
												{Object.keys(attendanceData[meet.id] || {}).filter(k => attendanceData[meet.id][parseInt(k)].status === "hadir").length} Hadir
											</Badge>
										</div>
									</AccordionTrigger>
									...
									<AccordionContent className="pt-4 pb-4 space-y-6">
												...
												{!isEditingInfo ? (
													<Button variant="ghost" size="sm" onClick={() => {
														setMeetingForm({
															...
															date: meet.meetingDate ? meet.meetingDate.split('T')[0] : "",
													...
													<div className="flex gap-2">
														<Button variant="ghost" size="sm" onClick={() => setEditingMeeting(null)}>Batal</Button>
														<Button size="sm" onClick={() => saveMeetingInfo(meet.id)}>
											...
											{isEditingInfo ? (
												<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
													<div className="space-y-2">
														...
														<Input className="border-2 border-slate-200" value={meetingForm.label} onChange={e => setMeetingForm({...meetingForm, label: e.target.value})} />
													</div>
													<div className="space-y-2">
														<Label>Tanggal Pertemuan</Label>
														<Input type="date" className="border-2 border-slate-200" value={meetingForm.date} onChange={e => setMeetingForm({...meetingForm, date: e.target.value})} />
													</div>
													<div className="space-y-2 md:col-span-2">
														<Label>Deskripsi / Materi / Topik</Label>
														<Textarea className="border-2 border-slate-200" value={meetingForm.desc} onChange={e => setMeetingForm({...meetingForm, desc: e.target.value})} rows={2} />
													</div>
												</div>
											) : (
												<div className="text-sm text-slate-600 space-y-1">
													<p><strong>Topik / Deskripsi:</strong> {meet.description || <span className="italic text-slate-400">Belum ada deskripsi</span>}</p>
												</div>
											)}
										</div>
											...
											<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
												{isUtsUas ? (
														<ActivityCard 
															...
															onSave={(actId?: number) => saveActivity(meet.id, "ujian", actId)}
														/>
												) : (
													<>
														<ActivityCard 
															...
															form={activityForm}
															setForm={setActivityForm}
															onSave={(actId?: number) => saveActivity(meet.id, "teori", actId)}
														/>
														<ActivityCard 
															title="Praktik" 
															type="praktik" 
															...
															form={activityForm}
															setForm={setActivityForm}
															onSave={(actId?: number) => saveActivity(meet.id, "praktik", actId)}
														/>
														<ActivityCard 
															title="Tugas" 
															type="tugas" 
															...
															form={activityForm}
															setForm={setActivityForm}
															onSave={(actId?: number) => saveActivity(meet.id, "tugas", actId)}
														/>
													</>
												)}
											</div>
										...
										<div className="space-y-3 pt-2">
											<div className="flex justify-between items-center border-b pb-2">
												<h3 className="font-semibold text-slate-700">Presensi Kelas</h3>
												<Button size="sm" onClick={() => saveAttendances(meet.id)} disabled={isSavingAttendance === meet.id}>
													{isSavingAttendance === meet.id ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Save className="h-4 w-4 mr-2" />}
													Simpan Presensi
												</Button>
											</div>
											...
											{students.length === 0 ? (
												<div className="text-center py-6 text-slate-500 text-sm bg-slate-50 rounded-md border border-dashed">
													Tidak ada mahasiswa aktif di angkatan {course.cohort}
												</div>
											) : (
												<div className="border rounded-md overflow-x-auto">
													<Table>
														...
														<TableBody>
															{students.map(s => {
																const att = attendanceData[meet.id]?.[s.id] || { status: "hadir", notes: "" };
																return (
																	<TableRow key={s.id}>
																		<TableCell className="font-medium text-slate-500">{s.nim}</TableCell>
																			...
																			<Select 
																				...
																				onValueChange={(val) => {
																					setAttendanceData(prev => {
																						const meetData = prev[meet.id] || {};
																						return {
																							...prev,
																							[meet.id]: {
																								...meetData,
																								[s.id]: { ...att, status: val || "hadir" }
																							}
																						};
																					});
																				}}
...
function ActivityCard({ title, type, meet, editingState, setEditing, form, setForm, onSave }: any) {
	const act = meet.activities.find((a: any) => a.activityType === type);
	const isEditing = editingState?.meetingId === meet.id && editingState?.type === type;
			...
			<div className="flex justify-between items-center mb-1">
				...
				{!isEditing ? (
					<Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => {
						setForm({ score: act?.score?.toString() || "", notes: act?.notes || "", documentUrl: act?.documentUrl, documentName: act?.documentName, file: null });
					...
					<div>
						...
						{form.documentUrl && !form.file && (
							<div className="flex items-center gap-2 p-2 bg-slate-50 border rounded-md text-xs mb-1">
								...
								<span className="flex-1 truncate">{form.documentName || "Dokumen tersimpan"}</span>
								<Button variant="ghost" size="sm" className="h-5 w-5 p-0" onClick={() => setForm((prev: any) => ({...prev, documentUrl: undefined, documentName: undefined}))}>
									<X className="h-3 w-3" />
								</Button>
							</div>
						)}
						<Input type="file" className="h-8 text-xs p-1" onChange={e => setForm({...form, file: e.target.files?.[0] || null})} />
					</div>
					<div className="flex gap-1 justify-end pt-1">
						<Button variant="ghost" size="sm" className="h-7 text-xs px-2" onClick={() => setEditing(null)}>Batal</Button>
						<Button size="sm" className="h-7 text-xs px-2" onClick={() => onSave(act?.id)}>Simpan</Button>
					</div>
				</div>
			) : (
				<div className="space-y-2">
					<div className="flex justify-between items-center">
						...
						<Badge variant="outline" className={act?.score ? "font-bold" : "text-slate-400 font-normal"}>
							{act?.score !== null && act?.score !== undefined ? act.score : "-"}
						</Badge>
					</div>
					<div className="text-xs text-slate-600 bg-slate-50 p-2 rounded border border-slate-100 min-h-[40px]">
						{act?.notes || <span className="italic text-slate-400">Belum ada catatan</span>}
					</div>
					{act?.documentUrl && (
						<div className="mt-2">
							<a 
