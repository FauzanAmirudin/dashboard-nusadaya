...
const NAV_LINKS = [
	...
	{ id: "status", label: "Status Akhir", roles: ["superadmin", "pmb", "crm", "finance", "akademik", "dosen", "pa", "magang", "evaluator"] },
	{ id: "final-decision", label: "Keputusan Final", roles: ["superadmin", "evaluator"] },
	{ id: "catatan", label: "Catatan", roles: ["superadmin"] },
];
	...
	return (
		<div className="pb-20 relative">
			...
			<div className="flex justify-between items-center mb-6">
				<Link
					href={user?.role === "superadmin" ? "/dashboard/students" : "/dashboard"}
			...
			<div className="mt-8 space-y-6">
				{mounted && (() => {
					const currentLink = visibleLinks.find(l => l.id === (activeTab || visibleLinks[0]?.id));
					if (!currentLink) return null;
					
					return (
						<div id={`panel-${currentLink.id}`} key={currentLink.id} className="animate-in fade-in duration-300">
							{currentLink.id === "pmb" ? (
								<PmbPanel
									studentId={s.id}
									pmbData={data.pmb}
								...
								<Card className="bg-white border-slate-200 shadow-sm">
									...
									<CardContent className="py-12 flex flex-col items-center justify-center text-slate-400">
										...
										<p>Konten Panel {currentLink.label} akan di-inject di sini.</p>
									</CardContent>
								</Card>
							)}
						</div>
				...
				<>
					...
					<div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 overflow-hidden relative">
						...
						<div className="flex flex-wrap gap-4 mb-8">
							{renderStamp("PMB", data.pmb?.status === "AMAN", "07/06/26")}
							{renderStamp("CRM", data.crm?.status === "AMAN", "07/06/26")}
							{renderStamp("Finance", data.finance?.status === "AMAN")}
							{renderStamp(
								"Akademik",
								data.academic?.status === "AMAN",
								"07/06/26",
							)}
							{renderStamp("Dosen", false)}
							{renderStamp("PA", false)}
							{renderStamp("Magang", data.internship?.status === "AMAN")}
						</div>
