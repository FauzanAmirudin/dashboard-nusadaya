...
export function PmbDashboard({ data, searchQuery, setSearchQuery, user }: any) {
	...
	const countPerhatian = data.filter((s: any) => s.pmb?.status === "PERLU_PERHATIAN" || !s.pmb?.status).length;
	const countTidakAman = data.filter((s: any) => s.pmb?.status === "TIDAK_AMAN").length;
		...
		<div className="space-y-6 pb-10">
			...
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
				<div>
					...
					<p className="text-slate-500 mt-1 text-sm">
						Selamat datang, {user.username}. Berikut ringkasan data penerimaan mahasiswa.
					</p>
				</div>
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
				...
				<Card className="bg-white border-slate-200 shadow-sm border-l-4 border-l-emerald-500">
					<CardContent className="p-5 flex items-start gap-4">
						...
						<div>
							<p className="text-slate-500 text-sm font-medium">Data PMB Lengkap (Aman)</p>
				...
				<Card className="bg-white border-slate-200 shadow-sm border-l-4 border-l-amber-500">
					<CardContent className="p-5 flex items-start gap-4">
						...
						<div>
							<p className="text-slate-500 text-sm font-medium">PMB Proses (Perhatian)</p>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
				...
				<Card className="bg-white border-slate-200 shadow-sm col-span-1 lg:col-span-2">
					<CardHeader>
						<CardTitle className="text-slate-800">Tabel Kelengkapan Mahasiswa</CardTitle>
					</CardHeader>
					<CardContent>
						<div className="mb-4">
							<Input
							...
							<Table>
								<TableHeader className="bg-slate-50 sticky top-0 z-10 shadow-sm">
									<TableRow className="border-slate-200 hover:bg-slate-50">
										...
										<TableHead className="text-slate-500 font-semibold text-center py-3">Status PMB</TableHead>
