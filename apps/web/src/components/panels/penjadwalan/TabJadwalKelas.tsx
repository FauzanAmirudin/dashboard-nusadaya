...
export function TabJadwalKelas({ canEdit }: { canEdit: boolean }) {
	...
	const handleDeleteConfirm = async () => {
		if (!deleteId) return;
		try {
			const { error } = await (api as any).scheduling["class-schedules"][
				deleteId.toString()
			].delete();
			if (!error) {
				...
				<div className="p-4 sm:p-5 border-b border-slate-100 flex flex-col gap-4">
					<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
						<div className="flex items-center gap-3">
							<div className="w-9 h-9 rounded-xl bg-blue-50 text-[#0517B0] flex items-center justify-center font-bold text-sm shadow-2xs shrink-0">
							...
							<div>
								...
								<p className="text-xs text-slate-500">
									Alokasi waktu dan ruangan mata kuliah perkuliahan reguler
								</p>
							</div>
						</div>
					...
					{isLoading ? (
						...
						<div className="flex flex-col items-center justify-center py-16 text-center px-4">
							...
							<p className="text-xs text-slate-500 max-w-sm mt-1 mb-4">
								{hasActiveFilters
									? "Tidak ada jadwal yang cocok dengan kata kunci atau filter yang dipilih."
									: "Belum ada data jadwal kelas perkuliahan yang ditambahkan."}
							</p>
							{hasActiveFilters ? (
								<Button
									variant="outline"
				...
				<AlertDialogContent className="rounded-2xl">
					<AlertDialogHeader>
						...
						<AlertDialogDescription className="text-xs text-slate-600">
							Apakah Anda yakin ingin menghapus jadwal kelas perkuliahan ini?
...
function ClassScheduleDialog({
	...
	const handleSave = async () => {
		if (
			...
			toast.error(
				"Mohon lengkapi data wajib (Mata Kuliah, Angkatan, Ruangan, Hari, dan Waktu)",
			);
			return;
		}
		...
		try {
			...
			let error;
			if (schedule?.id) {
				const res = await (api as any).scheduling["class-schedules"][
					schedule.id.toString()
				].patch(payload);
				error = res.error;
			} else {
				const res = await (api as any).scheduling["class-schedules"].post(
					payload,
				);
				error = res.error;
	...
	return (
		<Dialog open={true} onOpenChange={(open) => !open && onClose()}>
			<DialogContent className="sm:max-w-xl rounded-2xl max-h-[90vh] overflow-y-auto">
				<DialogHeader>
					<div className="flex items-center gap-3">
						<div className="w-10 h-10 rounded-xl bg-blue-50 text-[#0517B0] flex items-center justify-center font-bold shadow-2xs shrink-0">
							<BookOpen className="w-5 h-5" />
						</div>
						<div>
							<DialogTitle className="text-base font-bold text-slate-900">
								{schedule ? "Edit Jadwal Kelas" : "Tambah Jadwal Kelas"}
							</DialogTitle>
							<p className="text-xs text-slate-500 mt-0.5">
								Konfigurasi mata kuliah, pengampu, waktu, dan ruangan
							</p>
						</div>
					</div>
				</DialogHeader>
				...
				<div className="grid grid-cols-2 gap-3.5 py-3">
					<div className="space-y-1.5 col-span-2">
						...
						<Input
							...
							placeholder="Contoh: Bahasa Jepang Bisnis I"
					...
					<div className="space-y-1.5 col-span-2 sm:col-span-1">
						...
						<Input
							...
							placeholder="Contoh: Ruang Teori 2"
							className="h-9 text-xs rounded-xl"
						/>
					</div>
					...
					<div className="space-y-1.5 col-span-2">
						...
						<Textarea
							...
							placeholder="Tambahkan catatan khusus untuk sesi perkuliahan ini jika ada..."
							rows={2}
