...
export function TabJadwalPraktikum({ canEdit }: { canEdit: boolean }) {
	...
	const fetchSchedules = async () => {
		...
		try {
			...
			const { data, error } = await (api as any).scheduling["practicum-schedules"].get({
				$query: query,
			});
	...
	const handleDeleteConfirm = async () => {
		if (!deleteId) return;
		try {
			const { error } = await (api as any).scheduling["practicum-schedules"][deleteId.toString()].delete();
			if (!error) {
			...
			<Card>
				...
				<CardContent className="space-y-4">
					...
					{isLoading ? (
						...
						<div className="text-center py-8 text-slate-500">Tidak ada jadwal praktikum ditemukan.</div>
					) : (
...
function PracticumScheduleDialog({ dosens, schedule, onClose, onSuccess }: { dosens: any[], schedule: any, onClose: () => void, onSuccess: () => void }) {
	...
	const handleSave = async () => {
		...
		try {
			...
			let error;
			if (schedule?.id) {
				const res = await (api as any).scheduling["practicum-schedules"][schedule.id.toString()].patch(payload);
				error = res.error;
			} else {
				const res = await (api as any).scheduling["practicum-schedules"].post(payload);
				error = res.error;
	...
	return (
		<Dialog open={true} onOpenChange={(open) => !open && onClose()}>
			<DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
				<DialogHeader>
					<DialogTitle>{schedule ? "Edit Jadwal Praktikum" : "Tambah Jadwal Praktikum"}</DialogTitle>
				</DialogHeader>
				<div className="grid grid-cols-2 gap-4 py-4">
					<div className="space-y-2 col-span-2 sm:col-span-1">
						<Label>Mata Kuliah Praktik <span className="text-red-500">*</span></Label>
						<Input value={form.subject} onChange={e => setForm({...form, subject: e.target.value})} placeholder="Nama mata kuliah praktik" />
					</div>
					<div className="space-y-2 col-span-2 sm:col-span-1">
